/*
You're trying to add PRIMARY KEY(aspectId,aspectSkillId) to 'aspectSkillsToAspects' table
...
*/
ALTER TABLE `aspectSkillsToAspects` RENAME TO `old_aspectSkillsToAspects`;
--> statement-breakpoint
CREATE TABLE `aspectSkillsToAspects` (
	`aspectSkillId` text NOT NULL,
	`aspectId` text NOT NULL,
	PRIMARY KEY (`aspectId`, `aspectSkillId`)
);
--> statement-breakpoint
INSERT INTO `aspectSkillsToAspects` SELECT * FROM `old_aspectSkillsToAspects`;
--> statement-breakpoint
DROP TABLE `old_aspectSkillsToAspects`;
