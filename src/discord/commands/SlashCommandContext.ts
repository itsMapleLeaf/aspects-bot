...
export const SlashCommandContext = new Context((client: Discord.Client) => {
	const commands = new Map<CommandName, SlashCommand>()

	const autocompletions = new Map<
		`${CommandName}:${OptionName}`,
		Autocompletion
	>()

	client.on("ready", async (client) => {
		const data = () => map(commands.values(), (command) => command.data)
	...
	client.on("interactionCreate", async (interaction) => {
		...
		if (interaction.isAutocomplete()) {
			...
			const autocompletion = autocompletions.get(key)
			if (!autocompletion) {
				Logger.error((f) => {
					const available = join(
						map(autocompletions.keys(), (key) => "- " + f.highlight(key)),
						"\n",
					)
	...
	function addCommand<T extends SlashCommand>(command: T) {
		commands.set(command.data.name, command)
		return command
	}

	function addAutocompletion<T extends Autocompletion>(
		commandName: CommandName,
		optionName: OptionName,
		autocompletion: T,
	) {
		autocompletions.set(`${commandName}:${optionName}`, autocompletion)
		return autocompletion
	}

	return { addCommand, addAutocompletion }
})
